...
def gerar_ssim(csv_path, codigo_iata, output_file=None):
    try:
        ...
        # Alterar o formato da data de emissão para "YYYYMMDD"
        data_emissao = datetime.now().strftime("%Y%m%d")

        # Construir o nome do arquivo no novo padrão
        if output_file is None:
            output_file = f"{codigo_iata} {data_emissao} {data_min}-{data_max}.ssim"
        ...
        with open(output_file, 'w') as file:
            ...
            for i in range(4):
                if i == 3:
                    # Última linha de zeros (linha 4) com número ajustado
                    numero_linha_str = f"{numero_linha:06}E"
                    zeros_length = 200 - len(numero_linha_str)
                    zeros_line = "0" * zeros_length + numero_linha_str
                    file.write(zeros_line + "\n")
                else:
                    zeros_line = "0" * 200
                    file.write(zeros_line + "\n")
