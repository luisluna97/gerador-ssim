...
def gerar_ssim(csv_path, codigo_iata, output_file=None):
    try:
        ...
        df['Data'] = pd.to_datetime(df['Início'].str.split(" ").str[0], format="%d/%m/%Y")

        # Determinar datas mínima e máxima no CSV
        data_min_date = df['Data'].min()
        data_max_date = df['Data'].max()
        ...
        with open(output_file, 'w') as file:
            ...
            df_sorted = df.sort_values(by=['Voo', 'Data'])
            ...
            for _, row in df_sorted.iterrows():
                ...
                data_chegada = data.strftime("%d%b%y").upper()
                ...
                flight_date_key = (numero_voo_padded, data)
                ...
                date_counters[flight_key + '_last_date'] = data
            ...
            espacos_necessários = 200 - len(linha_5_conteudo) - len(numero_linha_str)
            linha_5 = linha_5_conteúdo + (' ' * espacos_necessários) + numero_linha_str
            file.write(linha_5 + "\n")
            numero_linha += 1
