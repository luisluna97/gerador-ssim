...
def gerar_ssim(csv_path, codigo_iata, output_file=None):
    try:
        ...
        with open(output_file, 'w') as file:
            ...
            # Linhas de zeros
            for i in range(4):
                if i == 3:  # Penúltima linha
                    penultima_linha_numero = f"{numero_linha:06}E"
                    zeros_length = 200 - len(penultima_linha_numero)
                    zeros_line = "0" * zeros_length + penultima_linha_numero
                    file.write(zeros_line + "\n")
                else:
                    zeros_line = "0" * 200
                    file.write(zeros_line + "\n")
